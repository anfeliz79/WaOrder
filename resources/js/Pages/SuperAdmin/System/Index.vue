...
async function refreshStatus() {
    ...
    try {
        const { data } = await axios.get('/superadmin/system/status');
        status.value = data;
    } finally {
...
async function loadLogs() {
    ...
    try {
        const { data } = await axios.get('/superadmin/system/logs');
...
<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Sistema</h1>
                <p class="text-sm text-gray-500 mt-0.5">Estado del servidor, deploy a produccion y mantenimiento</p>
            </div>
            <button
                @click="refreshStatus"
                :disabled="refreshing"
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-semibold text-gray-900">Checklist para produccion</h3>
                        <p class="text-xs text-gray-400">Completa cada paso antes de hacer deploy.</p>
                    </div>
                </div>
                ...
                <div v-for="item in (status.checklist || [])" :key="item.key" class="px-5 py-3">
                    <div class="flex items-center gap-3">
                        ...
                        <button
                            ...
                            :class="expandedHelp === item.key ? 'bg-amber-50 text-amber-600' : 'text-gray-400 hover:text-amber-500 hover:bg-gray-50'"
                        >
                            {{ expandedHelp === item.key ? 'Cerrar' : '¿Que es esto?' }}
                        </button>
                    </div>
                    <div v-if="expandedHelp === item.key"
                         class="mt-2 ml-9 p-3 rounded-lg bg-blue-50 border border-blue-100 text-sm text-blue-800 leading-relaxed">
                        {{ item.help }}
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4" :class="{ 'border-red-200 bg-red-50': !status.database?.ok }">
                <div class="flex items-center justify-between">
            ...
            <div class="bg-white rounded-xl border p-4" :class="status.pending_migrations > 0 ? 'border-amber-300 bg-amber-50' : 'border-gray-200'">
                <div class="flex items-center justify-between">
                    <p class="text-xs font-medium uppercase tracking-wide" :class="status.pending_migrations > 0 ? 'text-amber-600' : 'text-gray-500'">Migraciones</p>
                ...
                <p class="mt-1 text-base font-semibold" :class="status.pending_migrations > 0 ? 'text-amber-700' : 'text-gray-900'">
                    {{ status.pending_migrations > 0 ? `${status.pending_migrations} pendiente${status.pending_migrations > 1 ? 's' : ''}` : 'Al dia' }}
                </p>
            </div>
            <div class="bg-white rounded-xl border p-4" :class="!status.storage_link ? 'border-red-200 bg-red-50' : 'border-gray-200'">
                <div class="flex items-center justify-between">
                    <p class="text-xs font-medium uppercase tracking-wide" :class="!status.storage_link ? 'text-red-600' : 'text-gray-500'">Storage</p>
                    <CheckCircle v-if="status.storage_link" class="w-4 h-4 text-emerald-500" />
                    <XCircle v-else class="w-4 h-4 text-red-500" />
                </div>
                <p class="mt-1 text-base font-semibold" :class="!status.storage_link ? 'text-red-700' : 'text-gray-900'">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
                ...
                <button @click="runAction('migrate', '/superadmin/system/migrate')" :disabled="actions.migrate.loading"
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
                ...
                <button @click="runAction('clearCache', '/superadmin/system/cache/clear')" :disabled="actions.clearCache.loading"
                ...
                <button @click="runAction('rebuildCache', '/superadmin/system/cache/rebuild')" :disabled="actions.rebuildCache.loading"
                    ...
                    {{ actions.rebuildCache.loading ? 'Reconstruyendo...' : 'Reconstruir cache' }}
                </button>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
                ...
                <button @click="runAction('restartWorkers', '/superadmin/system/queue/restart')" :disabled="actions.restartWorkers.loading"
                    ...
                    {{ actions.restartWorkers.loading ? 'Enviando senal...' : 'Reiniciar workers' }}
                </button>
                ...
                <button @click="runAction('flushFailed', '/superadmin/system/queue/flush')" :disabled="actions.flushFailed.loading || status.failed_jobs === 0"
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
                ...
                <button @click="runAction('storageLink', '/superadmin/system/storage/link')" :disabled="actions.storageLink.loading"
                    class="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
        ...
        <div class="bg-white rounded-xl border border-gray-200">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-gray-400">ultimas 150 lineas</span>
                </div>
            ...
            <div v-if="logsVisible">
                ...
                <div v-else class="overflow-auto max-h-96 font-mono text-xs p-4 bg-gray-950 rounded-b-xl">
                    <div v-for="(line, i) in logs" :key="i" :class="logLineClass(line)"
